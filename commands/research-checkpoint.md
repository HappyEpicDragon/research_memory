...
3. 如果这次会话里有需要单独记一笔的决策（否决了什么方向、确认了什么、改变了什么判断），在 `.research/decisions.log.md` 追加一行。
4. 如果这次会话跑了新实验，确认 `.research/experiments.log.md` 有对应的一行索引（`outputs/...` 路径 + 一句话结论）。
5. 完成后用一两句话告诉用户这次 `state.md` 记录了什么、和之前比改了什么。
